<div align="center">
  ...
  <h3>✨ Created by <a href="https://github.com/ntkhang03">NTKhang</a> | Modified & Enhanced by <a href="https://github.com/lazyneoaz">NeoKEX</a></h3>
  
  <p>
    <strong>🚀 No Google Credentials Required!</strong><br>
    <em>Uses <a href="https://github.com/lazyneoaz/nkxfca">@lazyneoaz/nkxfca</a> for seamless Facebook integration</em>
  </p>
</div>
...
<div align="center">
  ...
  | 🛡️ **Render-Safe Memory** | 350 MB cleanup threshold + `--max-old-space-size=400` — never OOM-killed on Render free tier |
...
> - This bot uses [@lazyneoaz/nkxfca](https://github.com/lazyneoaz/nkxfca) (Facebook Chat API)
...
### 🟣 Render Free Tier — Memory Safety

Render's free web service has a **512 MB RAM limit**. This bot is tuned to stay well under it:

| Protection | What it does |
|------------|--------------|
| `--max-old-space-size=400` | Caps V8 heap at 400 MB; Node GCs aggressively before the container OOM-kills |
| `--expose-gc` | Lets the MemoryManager force a full GC when heap crosses 350 MB |
| **350 MB heap threshold** | Cleanup runs at 350 MB — 162 MB of headroom before Render's limit |
| **3-min check interval** | Memory is inspected every 3 minutes (down from 5) |
| **TTLMap auto-expiry** | All caches have TTL + max-size caps; nothing grows unbounded |
| **Session cleanup** | Dashboard MemoryStore sessions pruned hourly |

**Render start command:** `node index.js` *(the memory flags are already baked into `index.js`)*

> If you ever see `Process exited with code 137` in Render logs, that is an OOM kill. Lower `--max-old-space-size` to `350` in `index.js` if it happens.
...
- **[NeoKEX](https://github.com/lazyneoaz)**
